...
/**
 * Resilient JSON parsing helper: Extract JSON substring by finding matching braces/brackets.
 */
...
/**
 * Resilient JSON parsing helper: Repair truncated JSON by closing open quotes, objects, and arrays.
 */
...
/**
 * Helper to recursively build default values from a JSON Schema.
 */
...
/**
 * Helper to recursively merge parsed data with the schema default object.
 */
...
/**
 * Resilient JSON parsing function that extracts, repairs, parses, and aligns JSON output with defaults.
 */
function safeParseJson(text, cleanedText, schema = null) {
  let parsed = null;
  const defaults = schema ? getDefaultsFromSchema(schema) : null;
  
  // Step 1: Clean and Extract JSON Substring
  const targetText = cleanedText || text;
  const extracted = extractJson(targetText);
  if (!extracted) {
    console.error('GeminiParser: No valid JSON object or array detected in the response.');
    return defaults || {};
  }
  ...
  try {
    ...
    console.warn('GeminiParser: Standard JSON.parse failed. Attempting truncated JSON repair...');
    
    // Step 3: Attempt repair on failure (handles truncation)
    const repaired = repairTruncatedJson(extracted);
    try {
      parsed = JSON.parse(repaired);
      console.log('GeminiParser: Truncated JSON successfully repaired and parsed.');
    } catch (repairError) {
      console.error('GeminiParser: JSON parsing and repair both failed.');
      console.error('Raw response text:', text);
      console.error('Cleaned/Repaired text that failed parsing:', repaired);
      // Suppress crash by falling back to schema defaults
      parsed = null;
    }
  }
  
  // Step 4: Ensure all schema properties are set to prevent downstream crashes
  if (schema && defaults) {
    parsed = mergeWithDefaults(parsed, defaults);
  }
...
async function callAIProvider(prompt, systemInstruction = '', schema = null) {
  ...
  if (aiProvider === 'gemini') {
    ...
    if (isOpenRouter) {
      ...
      return schema ? safeParseJson(text, cleanedText, schema) : cleanedText;
    } else {
      ...
      // Fix: Call cleanJsonResponse on native Gemini too for robust formatting/quotes cleanup!
      const cleanedText = cleanJsonResponse(text);
      return schema ? safeParseJson(text, cleanedText, schema) : text;
    }
    ...
    if (schema) {
      messages.push({
        role: 'user',
        content: `Output MUST be valid JSON matching the schema: ${JSON.stringify(schema)}\nDo not include any chat prefix or suffix. Return ONLY the raw JSON object.`
      });
    }
    ...
    const ollamaFetch = async (body) => {
      ...
      try {
        ...
        if (err.name === 'AbortError') {
          throw new Error('Ollama request timed out after 5 minutes. The model may be overloaded or the resume is too large.');
        }
        throw err;
      }
    };
    ...
    if (schema) {
      try {
        ...
        const extracted = extractJson(testClean);
        JSON.parse(extracted);
      } catch (truncErr) {
        // Retry if it appears truncated or malformed on first try
        console.warn('Ollama: Response parsing failed or appeared truncated, retrying with extended token limit...');
        requestBody.options.num_predict = 65536;
        try {
          result = await ollamaFetch(requestBody);
          text = result.message?.content;
          if (!text) {
            console.warn('Ollama API returned an empty response on retry. Using original response.');
          }
        } catch (retryErr) {
          console.error('Ollama retry failed:', retryErr.message);
        }
      }
    }
